...
@Component({
  selector: 'app-high-risk-reports',
  templateUrl: './high-risk-reports.component.html',
  styleUrls: ['./high-risk-reports.component.css']
})
export class HighRiskReportsComponent implements OnInit {

  highRiskForm: FormGroup;
  ...
  createHighRiskForm() {
    this.highRiskForm = this.formBuilder.group({
  ...
  get startDate() {
    return this.highRiskForm.controls['startDate'].value;
  }

  get endDate() {
    return this.highRiskForm.controls['endDate'].value;
  }

  get agentID() {
    return this.highRiskForm.controls['agentID'].value;
  }
  ...
  searchReport() {
    let startDate: Date = new Date(this.highRiskForm.value.startDate);
    let endDate: Date = new Date(this.highRiskForm.value.endDate);
    ...
    console.log("Data form value...", JSON.stringify(this.highRiskForm.value));
    let reqObjForHighRiskReport = {
      ...
      "providerServiceMapID": this.highRiskForm.value.providerServiceMapID,
      "userID": this.highRiskForm.value.agentID,
      "fileName":  "High_Risk_Reports"
    }
    console.log("Data form data", JSON.stringify(reqObjForHighRiskReport, null, 4));
    this.reportService.getHighRiskReports(reqObjForHighRiskReport).subscribe((response) => {
      // console.log("Json data of response: ", JSON.stringify(response, null, 4));
      if (response) {
        saveAs(response,  reqObjForHighRiskReport.fileName+".xlsx");
        this.alertService.alert(this.currentLanguageSet.highRiskReportDownloaded);
  ...
  //       this.highRiskList = response.data;
  //       this.getResponseOfSearchThenDo();
  //     }
  //   })
  // }
  ...
  exportToxlsx(criteria: any) {
    if (this.highRiskList.length > 0) {
      let array = this.highRiskList.filter(function (obj) {
        for (var key in obj) {
          if (obj[key] == null) {
            obj[key] = "";
          }
      ...
      if (array.length != 0) {
        ...
        let wb_name = "High Risk Report";
        const criteria_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(criteria);
        const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.highRiskList, { header: (head) });
