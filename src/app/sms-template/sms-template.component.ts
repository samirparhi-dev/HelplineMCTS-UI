...
export class SmsTemplateComponent implements OnInit, DoCheck {
  ...
  ngOnInit() {
    ...
    this.providerServiceMapID = this.commonData.currentService.serviceID; // check for each module
          ...
          for (let i = 0; i < this.existing_templates.length; i++) {
            if (this.existing_templates[i].deleted === false) {
              this.smsType_ID_array.push(
                this.existing_templates[i].smsType.smsTypeID
              );
            }
          }
        }
  ...
  getSMSparameters() {
    ...
    this.sms_service.getSMSparameters().subscribe(
      (response) => {
