...
export class SupervisorAlertsNotificationsComponent implements OnInit, DoCheck {
  ...
  getServiceProviderID(providerServiceMapID) {
    this.notification_service
      .getServiceProviderID(providerServiceMapID)
      .subscribe(
        (response) => this.getServiceProviserIDSuccessHandeler(response),
  ...
  getAllNotificationTypes(providerServiceMapID) {
    this.notification_service
      ...
      .subscribe(
        (response) => this.getAllNotificationTypesSuccessHandeler(response),
        (err) => this.handleError(err)
      );
  }
  ...
  getAllNotificationTypesSuccessHandeler(response) {
    ...
    if (response.length === 0) {
      ...
      this.types = response.filter((item) => {
        return (
          item.notificationType.toUpperCase() === "Alert".toUpperCase() ||
  ...
  getOffices(roleID) {
    ...
    this.notification_service
      .getOfficeByRole(this.providerServiceMapID, roleID)
      .subscribe(
        (response) => this.getOfficeSuccessHandeler(response),
  ...
  getOfficeSuccessHandeler(response) {
    ...
    if (response.length == 0) {
      this.dialogService.alert(
        this.currentLanguageSet.noOfficeFoundWithTheSelectedRoleFunctionalInThem
      );
    } else {
  ...
  getNotifications(form_values) {
    ...
    this.notification_service.getSupervisorNotifications(obj).subscribe(
      (response) => {
        if (response.length != 0) {
  ...
  createAlertNotification(form_values) {
    ...
    let roleID =
      form_values.role == undefined ||
      form_values.role == null ||
      form_values.role == "" ||
      form_values.role == "All"
        ? undefined
        : form_values.role;
    defaultObj["roleID"] = roleID;

    let allOfficeIDs =
      form_values.offices == undefined ||
      form_values.offices == null ||
      form_values.offices == ""
        ? undefined
        : form_values.offices;
    ...
    if (roleID == undefined && allOfficeIDs == undefined) {
      for (var i = 0; i < this.roles.length; i++) {
        roomArray.push(
          this.providerServiceMapID + "_" + this.roles[i].roleName
        );
      }
      ...
      roomArray.push(
        this.providerServiceMapID + "_" + selectedRole[0].roleName
      );
    } else {
      let selectedRole = this.roles.filter((item) => {
        return item.roleID == roleID;
    ...
    this.notification_service.createNotification(requestArray).subscribe(
      (response) => {
        ...
        if (response.data != undefined && response.data.length > 0) {
            ...
            if (startDate.getTime() <= currentDate.getTime()) {
              this.notification_service
                .sendSocketNotification({
                  room: roomArray,
                  type: "Alert",
                  message: form_values.message.trim(),
                  subject: form_values.subject.trim(),
                })
                .subscribe(
                  (response) => {
          ...
          if (response.data[0].notificationTypeID == 19) {
            this.dialogService.alert(
              this.currentLanguageSet.notificationCreatedSuccessfully,
              "success"
            );
            this.notificationCreationForm.reset();
            if (startDate.getTime() <= currentDate.getTime()) {
      ...
      (error) => {
        console.log(error);
        this.dialogService.alert(
          this.currentLanguageSet.notificationAlertCreationFailedContactBackendTeam,
          "error"
        );
      }
    );
  ...
  validateTime(start_date, end_date, start_time, end_time) {
    ...
    if (
      ...
      end_time != undefined &&
      start_date.getDate() != undefined &&
      end_date.getDate() != undefined
    ) {
      if (
  ...
  edit(toBeEditedOBJ) {
    ...
    this.startTime =
      toBeEditedOBJ.validFrom.split("T")[1].split(":")[0] +
      ":" +
      toBeEditedOBJ.validFrom.split("T")[1].split(":")[1];
    this.endTime =
      toBeEditedOBJ.validTill.split("T")[1].split(":")[0] +
      ":" +
      toBeEditedOBJ.validTill.split("T")[1].split(":")[1];
  }
  ...
  editAlertNotification(form_values) {
    ...
    if (endDate > startDate) {
      ...
      this.notification_service.updateNotification(obj).subscribe(
        (response) => this.updateSuccess(response),
        (err) => this.notificationError(err)
      );
    } else {
      this.dialogService.alert(
        this.currentLanguageSet.validTillShouldBeAFutureDateThanValidFrom
      );
  ...
  updateSuccess(response) {
    ...
    if (response.data) {
      this.showTable();
      this.dialogService.alert(
        this.editType +" "+ this.currentLanguageSet.editedSuccessfully,
        "success"
      );

      this.refreshExistingTable(
    ...
    if (start_date == end_date) {
      if (start_time != undefined && end_time != undefined) {
        if (start_time > end_time) {
          this.dialogService.alert(
            this.currentLanguageSet.endTimeCantBeLessThanStartTime,
            "info"
          );
