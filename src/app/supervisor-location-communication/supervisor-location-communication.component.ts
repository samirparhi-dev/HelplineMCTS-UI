...
export class SupervisorLocationCommunicationComponent
  implements OnInit, DoCheck
{
  ...
  getAllNotificationTypesSuccessHandeler(response) {
    ...
    if (response.length == 0) {
      ...
      for (var k = 0; k < response.length; k++) {
        if (
          response[k].notificationType.toUpperCase() ===
  ...
  getOffices(psmID) {
    this.notification_service
      .getOffices(psmID)
      .subscribe((response) => this.getOfficesSuccessHandeler(response));
  }
  ...
  createLanguageMessage(form_values) {
    ...
    workingLocationIDs =
      form_values.offices == "" ? workingLocationIDs : form_values.offices;
    ...
    if (workingLocationIDs.length > 0) {
      for (var i = 0; i < workingLocationIDs.length; i++) {
        let defaultObj = {
          ...
          notification: (form_values.subject !== undefined && form_values.subject !== null) ? form_values.subject.trim() : null,
          notificationDesc: (form_values.message !== undefined && form_values.message !== null) ? form_values.message.trim() : null,
          validFrom: startDate,
          validTill: endDate,
        ...
        roomArray.push(
          this.providerServiceMapID + "_" + workingLocationIDs[i].toString()
        );
    ...
    this.notification_service.createNotification(request_array).subscribe(
      (response) => {
        console.log(response.data, "Location Message created");
        if (response.data.length > 0) {
          this.dialogService.alert(
            this.currentLanguageSet.locationMessageCreatedSuccessfully,
            "success"
          );
      ...
      (error) => {
        console.log(error);
        this.dialogService.alert(
          this.currentLanguageSet.locationMessageCreationFailedContactBackendTeam,
          "error"
        );
      }
    );
  ...
  updateSuccess(response) {
    ...
    if (response.data) {
      ...
      this.refreshExistingTable(
        this.location_communication_typeID,
